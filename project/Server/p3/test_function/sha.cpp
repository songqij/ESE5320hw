...
void sha256_hash(SHA256_CTX *ctx, const unsigned char *input, unsigned char *output, int flag, size_t length) {
    sha256_init(ctx);
...
int main() {
    ...
    const unsigned char *message = (const unsigned char *)"Hello, SHA-256!";  // Modify message type

    sha256_hash(&ctx, message, hash, 1, strlen((const char *)message));  // Pass the input as const unsigned char
